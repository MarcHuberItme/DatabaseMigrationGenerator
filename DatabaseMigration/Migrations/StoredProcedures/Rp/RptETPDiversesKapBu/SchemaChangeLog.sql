...
--changeset system:create-alter-procedure-RptETPDiversesKapBu context:any labels:c-any,o-stored-procedure,ot-schema,on-RptETPDiversesKapBu,fin-13659 runOnChange:true splitStatements:false stripComments:false endDelimiter:GO
--comment: Create stored procedure RptETPDiversesKapBu
CREATE OR ALTER PROCEDURE dbo.RptETPDiversesKapBu
@AcctPeriod int

AS

--DECLARE @AcctPeriod int
--SET @AcctPeriod = 201109
...
-- Noga-Code ermitteln 
UPDATE #Positions
SET Noga = ISNULL(a.Noga,'P')
--		, Bon = ISNULL(a.Bon,0), DatBon = ISNULL(a.DatBon,'19990101')
...
-- ETP-Positionen (mit ComptTypeText LIKE 'GR%') markieren, wenn Pendenz-Konto (A.GiltfürKonto) leer oder = pab.AccountId
UPDATE #Positions
SET ETP = 1
FROM #Positions AS pos
INNER JOIN (
				SELECT DISTINCT r.PositionId
				FROM
				(
				SELECT 	pos.Id As PositionId, ast2.TextShort AS KompT, pav.AccountNo AS GiltfuerKonto
				FROM PtOpenIssue poi
				JOIN PtBase ptb ON ptb.Id = poi.PartnerId
				JOIN PtPortfolio ppf ON ppf.PartnerId = ptb.Id
				JOIN PtAccountBase pab ON pab.PortfolioId = ppf.Id AND (pab.Terminationdate IS NULL OR pab.TerminationDate > @DateTo)
				JOIN PrReference ref ON ref.AccountId = pab.Id 
				JOIN PtPosition pos ON pos.ProdReferenceId = ref.Id
--				JOIN AcCompression2 ac2 ON ac2.PositionId = pos.Id AND ac2.AccountancyPeriod = @AcctPeriod
				JOIN AcCompression2 ac2 ON ac2.PositionId = pos.Id AND ac2.AccountancyPeriod BETWEEN @PrevAcctPeriod AND @AcctPeriod
				...
				LEFT OUTER JOIN PtAccountView pav ON pav.Id = poi.AccountId
				WHERE SubString(ac2.KeyString, 149, 7) <> '???????'
				AND SubString(ac2.KeyString, 64, 4) <> '????'
				AND CAST(SubString(ac2.keystring, 149, 7) AS int) = CAST(ppct.CompTypeNo AS int)
				AND pab.TerminationDate IS NULL
				AND poi.HdVersionNo < 999999999
				AND poi.StatusNo <> 15
				AND poi.TypeNo = 48
				AND ast2.TextShort LIKE 'GR%'
				AND (pav.AccountNo IS NULL OR pav.AccountNo = pab.AccountNo)
				) r
		) s ON pos.PositionId = s.PositionId
...
--		, p.Bon
		, p.Noga
		, 'ETP_Text' = CASE WHEN p.ETP = 1 THEN 'ETP-Kunden' ELSE 'Übrige Kunden' END
...
SELECT @NEU_Ueb_PH = SUM(Private_Haushalte) FROM #Book2 WHERE NOT ETP = 1 
SELECT @NEU_Ueb_UK = SUM(Übrige_Kunden) FROM #Book2 WHERE NOT ETP = 1 
...
SELECT @RZ_Ueb_PH = SUM(Private_Haushalte) FROM #Book3 WHERE NOT ETP = 1 
SELECT @RZ_Ueb_UK = SUM(Übrige_Kunden) FROM #Book3 WHERE NOT ETP = 1 
