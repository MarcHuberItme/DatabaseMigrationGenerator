...
--changeset system:create-alter-procedure-RptETPBonitaetKapBu context:any labels:c-any,o-stored-procedure,ot-schema,on-RptETPBonitaetKapBu,fin-13659 runOnChange:true splitStatements:false stripComments:false endDelimiter:GO
--comment: Create stored procedure RptETPBonitaetKapBu
CREATE OR ALTER PROCEDURE dbo.RptETPBonitaetKapBu
@AcctPeriod int
...
SELECT DISTINCT PositionId, AccountId, PartnerId, 'ETP' = 0, 'Noga' = '', 'Bon' = 0, 'DatBon' = '19900101' 
INTO #Positions 
FROM #Bookings 
ORDER BY PositionId
...
UPDATE #Positions
SET Noga = ISNULL(a.Noga,'P'), Bon = ISNULL(a.Bon,0), DatBon = ISNULL(a.DatBon,'19990101')
FROM #Positions AS pos
LEFT JOIN (	
	SELECT pos.PositionId
		, 'Noga' = CASE WHEN ISNULL(pb.NogaCode2008,'970000') = '970000' THEN 'P' ELSE 'U' END
		, ISNULL(prc.Result,0) As Bon
--		, CONVERT(char (10), ISNULL(prc.RatingDate,'19990101'), 104) As DatBon					-- DatLastRt, Datum letztes Rating
		, ISNULL(prc.RatingDate,'19990101') As DatBon					-- DatLastRt, Datum letztes Rating
	FROM #Positions pos
	INNER JOIN PtPosition pp ON pos.PositionId = pp.Id
	INNER JOIN PrReference ref ON pp.ProdReferenceId = ref.Id
	INNER JOIN PtAccountBase pab ON ref.AccountId = pab.Id
	INNER JOIN PtPortfolio ppf ON pab.PortfolioId = ppf.Id
	INNER JOIN PtBase pb ON ppf.PartnerId = pb.Id
	LEFT OUTER JOIN PtRatingConsolidation prc on prc.PartnerId = pb.Id 
			AND prc.RatingTypeNo = 12 and prc.HdVersionNo < 999999999
			AND prc.RatingDate = 
					(	
						SELECT MAX(prcsub.RatingDate)
						FROM PtRatingConsolidation prcsub
						WHERE prcsub.PartnerId = prc.PartnerId
						AND	prcsub.RatingTypeNo = 12 
						AND prcsub.HdVersionNo < 999999999
						AND prcsub.RatingDate <= @DateTo
					)
		
	WHERE (pab.TerminationDate IS NULL OR pab.terminationdate > @DateTo)
	AND	(pb.TerminationDate IS NULL OR pb.terminationdate > @DateTo)
		) a ON pos.PositionId = a.PositionId


-- ETP-Positionen (mit ComptTypeText LIKE 'GR%') markieren, wenn Bon(ität) 
UPDATE #Positions
SET ETP = 1
FROM #Positions AS pos
INNER JOIN AcCompression2 ac2 ON ac2.PositionId = pos.PositionId 
LEFT OUTER JOIN PrPrivateCompType ppct ON CAST(SubString(ac2.KeyString, 149, 7) AS int) = CAST(ppct.CompTypeNo AS int)
LEFT OUTER JOIN AsText ast2 ON ast2.MasterId = ppct.Id AND ast2.LanguageNo = 2
WHERE SubString(ac2.KeyString, 149, 7) <> '???????'
AND ast2.TextShort LIKE 'GR%'
AND pos.Bon < 2
...
Set  Noga = Case When (isnull(pb.NogaCode2008, '900000')) = 970000 Then 'P' Else 'U' END,
 ETP = Case When (Isnull (prc.Result, '9')) >= 2 Then 0 Else 1 END,
 BON = Isnull (prc.Result, '9') 
From #Positions AS pos
...
SELECT p.PositionId, p.PartnerId, 'Art' = 'Neugeschäfte'
		, p.ETP, p.Bon, p.Noga
		, 'ETP_Text' = CASE WHEN (p.ETP = 1 AND p.Bon < 2) THEN 'ETP-Kunden' ELSE 'Übrige Kunden' END
...
SELECT @NEU_ETP_PH = SUM(Private_Haushalte) FROM #Book2 WHERE (ETP = 1 AND Bon < 2)
SELECT @NEU_ETP_UK = SUM(Übrige_Kunden) FROM #Book2 WHERE (ETP = 1 AND Bon < 2)
SET @NEU_ETP_Tot = @NEU_ETP_PH + @NEU_ETP_UK

SELECT @NEU_Ueb_PH = SUM(Private_Haushalte) FROM #Book2 WHERE NOT (ETP = 1 AND Bon < 2)
SELECT @NEU_Ueb_UK = SUM(Übrige_Kunden) FROM #Book2 WHERE NOT (ETP = 1 AND Bon < 2)
...
SELECT @RZ_ETP_PH = SUM(Private_Haushalte) FROM #Book3 WHERE (ETP = 1 AND Bon < 2)
SELECT @RZ_ETP_UK = SUM(Übrige_Kunden) FROM #Book3 WHERE (ETP = 1 AND Bon < 2)
SET @RZ_ETP_Tot = @RZ_ETP_PH + @RZ_ETP_UK

SELECT @RZ_Ueb_PH = SUM(Private_Haushalte) FROM #Book3 WHERE NOT (ETP = 1 AND Bon < 2)
SELECT @RZ_Ueb_UK = SUM(Übrige_Kunden) FROM #Book3 WHERE NOT (ETP = 1 AND Bon < 2)
...
SELECT @Pt_ETP_PH = COUNT(DISTINCT PartnerId) FROM #Book2 WHERE (ETP = 1 AND Bon < 2 AND Noga = 'P')
SELECT @Pt_ETP_UK = COUNT(DISTINCT PartnerId) FROM #Book2 WHERE (ETP = 1 AND Bon < 2 AND Noga = 'U')
...
SELECT @Pt2_ETP_PH = COUNT(DISTINCT PartnerId) FROM #Book3 WHERE (ETP = 1 AND Bon < 2 AND Noga = 'P')
SELECT @Pt2_ETP_UK = COUNT(DISTINCT PartnerId) FROM #Book3 WHERE (ETP = 1 AND Bon < 2 AND Noga = 'U')
