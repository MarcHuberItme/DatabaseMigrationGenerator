...
--changeset system:create-alter-procedure-GetAccountWarnings context:any labels:c-any,o-stored-procedure,ot-schema,on-GetAccountWarnings,fin-13659 runOnChange:true splitStatements:false stripComments:false endDelimiter:GO
--comment: Create stored procedure GetAccountWarnings
CREATE OR ALTER PROCEDURE dbo.GetAccountWarnings
...
SELECT 	1 AS LineNumber, Id, 'PtInfoPoolAccount' AS ParentTableName, 1 AS IsWarning, HdChangeDate AS BlockDate, 
	HdChangeUser AS BlockIssuer,  NULL AS BlockReason, Remark AS BlockComment 
FROM PtInfoPoolAccount 
WHERE AccountId = @AccountId
AND HdVersionNo BETWEEN 1 AND 999999998 
AND EXISTS (	SELECT * FROM PtInfoPoolAccount WHERE AccountId = @AccountId
		AND HdVersionNo BETWEEN 1 AND 999999998
