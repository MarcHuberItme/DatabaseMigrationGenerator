...
--changeset system:create-alter-procedure-GetEbankingPositionListByPortfolioId context:any labels:c-any,o-stored-procedure,ot-schema,on-GetEbankingPositionListByPortfolioId,fin-13659 runOnChange:true splitStatements:false stripComments:false endDelimiter:GO
--comment: Create stored procedure GetEbankingPositionListByPortfolioId
CREATE OR ALTER PROCEDURE dbo.GetEbankingPositionListByPortfolioId
@InPortfolioId AS uniqueidentifier,
@InAgrEbankingId AS uniqueidentifier,
@InVaRunId AS uniqueidentifier,
@InLanguageNo AS tinyint,
@InOrderByField as varchar(50) ,
@InOrderAsc as bit,
@InPage AS int,
@InRowsPerPage AS int
...
DECLARE @OrderByField as varchar(50) 
DECLARE @OrderAsc as bit
DECLARE @Page AS int
...
SET @OrderByField = @InOrderByField
SET @OrderAsc = @InOrderAsc
SET @Page = @InPage
SET @RowsPerPage = @InRowsPerPage
...
ORDER BY 
       ...
       WHEN 1 THEN -- ASC 
             CASE @OrderByField 
             WHEN 'MarketValueChf'      THEN PosValView.MarketValueCHF 
             WHEN 'MarketValueVaCu'     THEN PosValView.MarketValueVaCu 
             WHEN 'Quantity'            THEN PosValView.ActualQuantity 
             WHEN 'ReturnOnInvestment'  THEN VaCvView.TotalProfitPfCuPercent 
             WHEN 'Rate'                THEN PosValView.Rate
             END
       END ASC,
       ...
       WHEN 1 THEN -- ASC 
             CASE @OrderByField 
             WHEN 'DueDate'             THEN PosValView.DueDate 
             END
       END ASC,
       ...
       WHEN 1 THEN -- ASC 
             CASE @OrderByField 
             WHEN 'NominalCurrency'     THEN Pub.NominalCurrency 
             WHEN 'PositionDescription' THEN PosValView.Description 
             WHEN 'InstrumentTypeText'  THEN InstrTxt.TextShort 
             END
       END ASC,
...
PosValView.Description, PosValView.Id
OFFSET @Offset Rows
