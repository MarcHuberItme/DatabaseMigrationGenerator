...
--changeset system:create-alter-procedure-GetAccountNoWhenEBankingConnectionIsValid context:any labels:c-any,o-stored-procedure,ot-schema,on-GetAccountNoWhenEBankingConnectionIsValid,fin-13659 runOnChange:true splitStatements:false stripComments:false endDelimiter:GO
--comment: Create stored procedure GetAccountNoWhenEBankingConnectionIsValid
CREATE OR ALTER PROCEDURE dbo.GetAccountNoWhenEBankingConnectionIsValid
...
DECLARE @AccountNo VARCHAR(38)

BEGIN
    --Giver = Receiver
    SELECT DISTINCT @AccountNo = PAB.AccountNo
    ...
    INNER JOIN PtAccountBase PAB ON PAB.PortfolioId = PF.Id
    ...
    BEGIN
        ...
        INNER JOIN PtAccountBase PAB ON PAB.Id = @AccountId
                   AND PAB.TerminationDate IS NULL
        INNER JOIN PtPortfolio PF ON PF.Id = PAB.PortfolioId       
                   AND PBReceiver.TerminationDate IS NULL
        INNER JOIN PtBase PBGiver ON PBGiver.Id = PF.PartnerId                        --Partner dessen Konto hinzugefügt werden soll
