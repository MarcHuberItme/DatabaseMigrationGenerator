...
--changeset system:create-alter-procedure-GetAdviceRulesByAccountId context:any labels:c-any,o-stored-procedure,ot-schema,on-GetAdviceRulesByAccountId,fin-13659 runOnChange:true splitStatements:false stripComments:false endDelimiter:GO
--comment: Create stored procedure GetAdviceRulesByAccountId
CREATE OR ALTER PROCEDURE dbo.GetAdviceRulesByAccountId
...
ELSE
   BEGIN
	SELECT @IsSpecificRule = 1 WHERE @DebitAdvice IS NOT NULL
	...
	SELECT ISNULL(@DebitAdvice,2) AS DebitAdviceTypeNo, 
		...
		ISNULL(@IsSpecificRule,0) AS IsSpecificRule
   END
