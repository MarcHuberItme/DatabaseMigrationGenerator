...
--changeset system:create-alter-procedure-AiTaxAssetGetFwgValue context:any labels:c-any,o-stored-procedure,ot-schema,on-AiTaxAssetGetFwgValue,fin-13659 runOnChange:true splitStatements:false stripComments:false endDelimiter:GO
--comment: Create stored procedure AiTaxAssetGetFwgValue
CREATE OR ALTER PROCEDURE dbo.AiTaxAssetGetFwgValue
...
FROM VaRun r 
    LEFT OUTER JOIN VaCurrencyRate c ON c.ValRunId = r.Id and  AccountCurrency = 'CHF' and ValuationCurrency = @Currency
...
SELECT @ValueCHF = SUM(MarketValueChf), @ValueFWG = SUM(MarketValueChf) * @VaCurrencyRate
FROM (
    -- Konten
	...
	WHERE ((p.ProductNo< 3000) OR (p.ProductNo >= 4000 AND p.ProductNo < 9000))
	   AND p.ProductNo NOT IN (1035,4047)
